...
$data->read('img/graduate_students.xls');
...
for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) {
	//for ($j = 1; $j <= $data->sheets[0]['numCols']; $j++) {
		//echo "\"".$data->sheets[0]['cells'][$i][$j]."\",";
			...
			/*for ($j = $i; $j <= $data->sheets[0]['numRows']; $j++){*/
			if($headcount!=1){
			...
			if($data->sheets[0]['cells'][$i][$col_name] !=""){
              ...
              echo "<p>".$data->sheets[0]['cells'][$i][$col_major]."</p>";
          ...
          if($count % 4 == 0){
          	...
          	echo "</div>";
				...
				$count=0;
				echo "</div>";
				echo "<h1 class='gs-year'>".$data->sheets[0]['cells'][$i+1][$col_year]."</h1>";
				//echo "<div class='row gs-row'>";
			}
	//}
	/*echo "\n";*/
